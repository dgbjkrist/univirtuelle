import { enseignants, activites, HEURES_NORMALES, getHeuresEnseignant, ressources } from "@/data/mockData";
...
const chartData = enseignants.map((e) => {
  ...
  return { name: `${e.prenom[0]}. ${e.nom}`, heures: h.total, complementaires: h.complementaires };
});
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex gap-2">
          <Button variant="outline" className="gap-2" onClick={() => handleExport("PDF")}><Download className="h-4 w-4" /> PDF</Button>
          <Button variant="outline" className="gap-2" onClick={() => handleExport("Excel")}><FileText className="h-4 w-4" /> Excel</Button>
                ...
                {enseignants.map((e) => {
                  ...
                  return (
                    <TableRow key={e.id}>
                      ...
                      <TableCell className="text-right font-medium">{(h.complementaires * e.tauxHoraire).toLocaleString()} DA</TableCell>
                    </TableRow>
                  );
                })}
              </TableBody>
